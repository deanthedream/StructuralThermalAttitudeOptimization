...
class ComponentPositions(object):
    def __init__(self):
        ...
        self.InertialFrame = {'x':[1.,0.,0.],'y':[0.,1.,0.],'z':[0.,0.,1.]} #the frame from which everything is defined
        ...
        with open(fname, 'rb') as f:#load STLfile
            comps = json.load(f)#f.readlines()

        #iterate over each part
        for part in comps.keys():
            #check if part DOES NOT HAVE body_vector assigned to it
            if not comps[part].has_key('part_position'):
                #assign body vector
                comps[part]['part_position'] = self.randomPartPositionVector()

            #check if part has part_body_frame
            if not comps[part].has_key('part_body_frame'):
                comps[part]['part_body_frame'] = self.randomPartBodyFrame()
    ...
    def randomPartPositionVector(self):
        ...
        theta = np.rangom.uniform(low=0,high=2*np.pi)
        h = np.random.uniform(low=0,high=self.FairingH)

        x = r*np.cos(theta)
        y = r*np.sin(theta)
        body_vector = np.array([x,y,h]) + self.r_SCbody_I#this should produce a spacecraft body_vector position that updates if teh spacecraft body updates
